/* 
   我们顺势来看看 dlr是什么来的，查看其代码dlr/main.c，代码功能一目了然，就是使用socket从目标IP下载 mirai病毒。
   首先调用open创建dvrHelper文件，然后使用socket连接黑客的文件服务器，获取[ip] / bins / mirai的数据，写入到dvrHelper文件中。
   dlr的作用就是，在目标IoT设备无法使用wget、tftp命令的时候，使用它来下载mirai病毒。
*/
#include <sys/types.h>
